...
export function WarehouseMapPage() {
  ...
  const loadWarehouse = async (wid: number) => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Не удалось загрузить данные склада');
    } finally {
      setLoading(false);
    }
  };
  ...
  const runRoute = async () => {
    ...
    if (skuIds.length === 0) { setRouteError('Введите хотя бы один ID артикула'); return; }
    ...
    try {
      ...
      setRouteError(e instanceof Error ? e.message : 'Ошибка оптимизации маршрута');
    } finally {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 16px' }}>
      <Title level={3} style={{ marginBottom: 4 }}>Карта склада</Title>
      <Paragraph type="secondary" style={{ marginBottom: 24 }}>
        Цвет ячеек отражает суммарный скор — красный = высокий приоритет (быстрые товары).
        Включите маршрут для визуализации оптимального пути сборки.
      </Paragraph>
        ...
        <Row gutter={24} align="middle" wrap>
          ...
          {graphData && (
            <Col style={{ marginTop: 20, marginLeft: 'auto' }}>
              <Space>
                <Text style={{ fontSize: 13 }}>Маршрут</Text>
            ...
            <Row gutter={16} align="bottom" wrap>
              <Col flex="auto">
                <Text style={{ fontSize: 12, color: '#595959', display: 'block', marginBottom: 4 }}>
                  ID артикулов (через запятую или пробел)
                </Text>
                <Input
                  placeholder="например 1, 2, 5, 12"
                  value={skuInput}
      ...
      {!loading && graphData && (
        <Row gutter={16}>
                ...
                {slots.map(slot => {
                  ...
                  return (
                    <g key={slot.id}>
                      ...
                      {pickIdx >= 0 ? (
                        <>
                          <circle cx={x + CELL - 5} cy={y + 5} r={5} fill="#F97316" style={{ pointerEvents: 'none' }} />
                          <text
                            x={x + CELL - 5}
                            y={y + 8}
                            textAnchor="middle"
          ...
          {hovered && (
            <Col style={{ width: 220 }}>
              <Card
                title={hovered.isDock ? 'Докинг' : hovered.label}
                size="small"
                style={{ position: 'sticky', top: 40 }}
              >
                {hovered.isDock ? (
                  <Text type="secondary" style={{ fontSize: 12 }}>
                    Точка приёма — все маршруты начинаются и заканчиваются здесь.
                  </Text>
                ) : (
                  <>
                    <div style={{ marginBottom: 8 }}>
                      <Text style={{ fontSize: 12, color: '#8c8c8c' }}>Позиция</Text>
                      <div>
                        <Text style={{ fontSize: 13 }}>
                          Ряд {hovered.row}, Кол {hovered.col}
                        </Text>
                      </div>
                    </div>
                    <div style={{ marginBottom: 8 }}>
                      <Text style={{ fontSize: 12, color: '#8c8c8c' }}>Скор</Text>
                      <div>
                        <Tag
                          color={
                    ...
                    {routeSlots.includes(hovered.id) && (
                      <div>
                        <Text style={{ fontSize: 12, color: '#8c8c8c' }}>Порядок сборки</Text>
                        <div>
                          <Tag color="orange">
                            Остановка #{routeSlots.indexOf(hovered.id) + 1}
                          </Tag>
                        </div>
                      </div>
                    )}
      ...
      {!loading && !graphData && !error && (
        <Card>
          <div style={{ textAlign: 'center', padding: 40, color: '#8c8c8c', fontSize: 14 }}>
            Введите ID склада выше для загрузки карты.
          </div>
        </Card>
      )}
    </div>
